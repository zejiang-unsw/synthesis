...
data.gen.unif<-function(n,ndim=9,r=0.6,sigma, method=c("pearson","spearman")){
  ...
  if(missing(sigma)){
    # Pearson correlation
    if(method=="spearman") rho <- 2 * sin(r * pi/6) else rho <- r

    #Correlation matrix
    sigma=matrix(rep(rho,ndim*ndim),ndim); diag(sigma) <- 1
  } else {
    if(ndim!=ncol(sigma)) stop("The dimension of sigma is different from ndim!")
  }
...
data.gen.norm<-function(n,mu=rep(0,2),sd=rep(1,2),r=0.6,sigma){
  ...
  if(missing(sigma)){
    ...
    if(length(mu)!=length(sd)) stop("The dimension of sd is different from mu!")

    #Covariance matrix
    sigma=matrix(rep(r,length(sd)*length(sd)),length(sd)); diag(sigma) <- 1
    sigma=sigma*prod(sd)

  } else{
    if(length(mu)!=ncol(sigma)) stop("The dimension of sigma is different from mu!")
  }
